...
function updatePriceFilter(min, max, isDesktop = false) {
  ...
  if (!priceFilterItem) {
    ...
    priceFilterItem.classList.add(
      isDesktop ? "filtre-item" : "filtre-item",
      "price"
    );
    ...
    filterContent.classList.add(
      isDesktop ? "filter-content-desktop" : "filter-content"
    );
...
function spanFonction1(spanId) {
  const icon = document.querySelector(
    `.filtre-span[span-desktop-id="${spanId}"]`
  );
  if (icon) {
    icon.classList.toggle("span-plus");
    icon.classList.toggle("span-minus");
...
function toggleRotation1(filtreId) {
  const icon = document.querySelector(
    `.icon-arrow[filtre-desktop-id="${filtreId}"]`
  );
  if (icon) {
    icon.classList.toggle(
      "rotated-plus",
      icon.classList.contains("rotated-minus")
    );
    icon.classList.toggle(
      "rotated-minus",
      !icon.classList.contains("rotated-minus")
...
let nbrInformatique = [0, 0, 0, 0, 0, 0];
...
let nbrInformatique_desktop = [0, 0, 0, 0, 0, 0];
...
let nbrPhone_desktop = [0, 0, 0, 0, 0];
...
function createFilterItem(name, checkbox, labelText, container, isDesktop) {
  ...
  removeBtn.addEventListener("click", () => {
    ...
    const relatedElements = document.querySelectorAll(
      `[data-type="${checkbox.getAttribute("data-type")}"]`
    );
    ...
    if (isDesktop) {
      if (contentsPhone_desktop.includes(name)) {
        ...
        if (nbrPhone_desktop[index] == 0) {
          const pho = document.querySelectorAll(
            `[data-type="informatique-desktop"]`
          );
...
function handleCheckboxes(checkboxes, container, isDesktop) {
  checkboxes.forEach((checkbox) => {
    checkbox.addEventListener("change", function () {
      const labelText = `${this.getAttribute(
        "name"
      )} - ${this.nextSibling.textContent.trim()}`;
      if (this.checked) {
        createFilterItem(
        ...
        if (this.filterItem) {
          ...
          if (isDesktop) {
            if (contentsPhone_desktop.includes(this.getAttribute("name"))) {
              ...
              const index = contentsPhone_desktop.indexOf(
                this.getAttribute("name")
              );
              nbrPhone_desktop[index] -= 1;
              console.log(nbrPhone_desktop[index]);
              ...
              contentsInformatique_desktop.includes(this.getAttribute("name"))
            ) {
              console.log(this.getAttribute("name"));
              const index = contentsInformatique_desktop.indexOf(
                this.getAttribute("name")
              );
              nbrInformatique_desktop[index] -= 1;
              console.log(nbrInformatique_desktop[index]);
            ...
            if (contentsPhone.includes(this.getAttribute("name"))) {
              ...
              contentsInformatique.includes(this.getAttribute("name"))
            ) {
              console.log(this.getAttribute("name"));
              const index = contentsInformatique.indexOf(
                this.getAttribute("name")
              );
              nbrInformatique[index] -= 1;
              console.log(nbrInformatique[index]);
...
function clearAllFilters1(checkboxes, container) {
  ...
  clearAllBtn.addEventListener("click", (e) => {
    ...
    nbrInformatique_desktop = [0, 0, 0, 0, 0, 0];

    nbrPhone_desktop = [0, 0, 0, 0, 0];
  });
}

// Gérer les sections mobile et desktop
const mobileCheckboxes = document.querySelectorAll(
  '.filter-container-mobile input[type="checkbox"]'
);
...
const desktopCheckboxes = document.querySelectorAll(
  '.filter-container-desktop input[type="checkbox"]'
);
const desktopSelectedFilters = document.querySelector(
  ".selectedFilters-desktop"
);
...
const desktopElementsA = document.querySelectorAll(
  '[data-type="informatique-desktop"]'
);
const desktopElementsB = document.querySelectorAll(
  '[data-type="phone-desktop"]'
);
      ...
      ProductsList.forEach(async (product) => {
        ...
        productElement.innerHTML = `
                    ...
                    <div class="product-info">
                        <p class="fs-poppins fs-50 bold-500 text-blue">${
                          product.name
                        }</p>
                    ...
                    <div class="detail">
                        ...
                        <div class="detail-text">
                            ...
                            <div class="detail-dispo">
                                <span class="${
                                  product.stock > 0 ? "text-green" : "text-red"
                                } fs-50">
                                    ${
                                      product.stock > 0
                                        ? "En stock"
                            ...
                            <div class="detail-el">
                                <i class="uil uil-shopping-cart-alt text-white add-to-cart" data-id="${
                                  product.id
                                }"></i>
                                <i class="bi bi-heart icon-heart text-white" data-id="${
                                  product.id
                                }"></i>
                            </div>
                        </div>
