  ...
  try {
    ...
    try {
      ...
      return {
        status: 500,
        message:
          "Erreur lors de la suppression du compte dans Firestore : " +
          error.message,
      };
    }
  } catch (error) {
    ...
    if (error.code === "auth/user-mismatch") {
      errorMessage =
        "Les informations d'identification ne correspondent pas à l'utilisateur connecté.";
    } else if (error.code === "auth/wrong-password") {
...
export function getUserChange(displayElementId) {
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      ...
      displayElement.innerHTML =
        '<a class="bbb text-black uil fs-150" href="/login.html"><i class="uil fs-150 uil-user"></i>Se connecter</a>';
    }
  });
}
...
export async function getUserNameByUid(uid) {
  ...
  if (userData) {
    ...
    if (adresse) {
      toggleLoadingSpinner(false);
      return `${capitalizeFirstLetter(adresse.prenom)} ${capitalizeFirstLetter(
        adresse.nom
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      if (cart[productId]) {
        ...
        console.log(
          `Quantité mise à jour pour le produit ${productId} : ${cart[productId].qty}`
        );
        ...
        console.log(
          `Produit ${productId} ajouté au panier avec une quantité de ${quantity}`
        );
        await getTotalQuantityInCart();
      }
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      if (cart[productId]) {
        ...
        console.log(
          `Produit ${productId} retiré du panier de l'utilisateur ${userId}`
        );
        await getTotalQuantityInCart();
      } else {
        console.log(
          `Produit ${productId} non trouvé dans le panier de l'utilisateur ${userId}`
        );
      }
    } else {
...
export const removeFavorite = async (productId) => {
  ...
  try {
    ...
    console.log(
      `Produit ${productId} supprimé des favoris de l'utilisateur ${userId}`
    );
  } catch (error) {
...
export const increaseQuantity = async (productId) => {
  ...
  try {
    ...
    console.error(
      "Erreur lors de l'augmentation de la quantité du produit",
      error
    );
  } finally {
    // toggleLoadingSpinner(false); // Masque le spinner après la requête
...
export const decreaseQuantity = async (productId) => {
  ...
  try {
    ...
    console.error(
      "Erreur lors de la diminution de la quantité du produit",
      error
    );
  } finally {
    // toggleLoadingSpinner(false); // Masque le spinner après la requête
...
export const getTotalQuantityInCart = async () => {
  onAuthStateChanged(auth, async (user) => {
    try {
      ...
      if (!userId) {
        console.error(
          "Aucun utilisateur connecté pour récupérer la quantité totale dans le panier"
        );
      ...
      console.error(
        "Erreur lors de la récupération de la quantité totale dans le panier",
        error
      );
      if (bagQuantityElement) {
        bagQuantityElement.setAttribute("data-quantity", 0); // Mettre à jour l'attribut
...
export async function updateProductStatusInOrderAdmin(
  orderId,
  newStatus,
  productIndex,
  verificationNote = ""
    ...
    onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (newStatus === "checking") {
          itemToUpdate.verificationNote =
            verificationNote || "Aucune note fournie";
        }
        ...
        const updates = {
          ...
          status: allCancelled
            ? "cancelled"
            : allSameStatus && orderData.status != newStatus
            ? newStatus
            : orderData.status,
        };
...
export async function updateProductStatusInOrder(
  orderId,
  newStatus,
  productIndex,
  verificationNote = ""
    ...
    onAuthStateChanged(auth, async (user) => {
      if (user) {
        try {
          ...
          if (newStatus === "checking") {
            items[productIndex].verificationNote =
              verificationNote || "Aucune note fournie";
          }
          ...
          resolve({
            status: 200,
            message:
              "Statut du produit et de la commande mis à jour avec succès",
          });
        } catch (error) {
          console.error("Erreur lors de la mise à jour :", error.message);
          reject({
